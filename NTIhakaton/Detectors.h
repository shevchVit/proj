...
#define GREEN_BALL_LOWER 50, 145, 110
#define GREEN_BALL_UPPER 80, 255, 255

#define YELLOW_BALL_LOWER 20, 70, 100
#define YELLOW_BALL_UPPER 50, 255, 255
...
Object DetectYellow1(cv::Mat &image)
{
    ...
    cv::RotatedRect result;
    cv::cvtColor(image, image, CV_BGR2HSV);
    cv::Scalar lower(YELLOW_BALL_LOWER); 
    cv::Scalar upper(YELLOW_BALL_UPPER); 
    cv::inRange(image, lower, upper, image);
    //cv::Mat cropedImage = image(cv::Rect(0, 0, 300, 100));
    //cv::imshow("bin", image);
    //cv::waitKey(10); 
    ...
    for(size_t i = 0; i < conturs.size(); i++){
        std::vector<cv::Point> hull;
        cv::convexHull(conturs[i], hull, true);
        cv::approxPolyDP(hull, hull, 5, true);
        std::vector<std::vector<cv::Point>> vec;
        vec.push_back(hull);
        if (hull.size() < 3) continue;  
        if(conturs.at(i).size() < 5) continue;
...
Object detectGreenBall(cv::Mat &image)
{
    Object obj;
    std::vector<cv::vector<cv::Point>> conturs;
    //cv::Mat fin = image.clone();
    ...
    cv::RotatedRect result;
    cv::cvtColor(image, image, CV_BGR2HSV);
    cv::Scalar lower(GREEN_BALL_LOWER);
    cv::Scalar upper(GREEN_BALL_UPPER);
    cv::inRange(image, lower, upper, image);
    //cv::imshow("1", image);
    //cv::waitKey(10);
    cv::findContours(image, conturs, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
